...
{
    public TradingPairRepository(ApplicationDbContext dbContext) : base(dbContext) { }

    public async Task<TradingPair?> GetBySymbolAsync(string symbol)
    {
        return await _dbContext.Set<TradingPair>().AsNoTracking()
    ...
    {
        return await _dbContext.Set<TradingPair>().AsNoTracking()
            .Where(tp => tp.IsActive).OrderBy(tp => tp.Symbol).ToListAsync();
    }
    ...
    public async Task<bool> UpdatePriceAsync(string symbol, decimal price, decimal change24h, decimal volume24h, decimal high24h, decimal low24h)
    {
        ...
        var rows = await _dbContext.Database.ExecuteSqlInterpolatedAsync(
            $"UPDATE TradingPairs SET Price={price}, Change24h={change24h}, Volume24h={volume24h}, High24h={high24h}, Low24h={low24h}, UpdatedAt={now}, LastUpdated={now} WHERE Symbol={symbol} AND IsDeleted=0");
        return rows > 0;
    }
    ...
    {
        return await _dbContext.Set<TradingPair>().AsNoTracking()
            .Where(tp => tp.IsActive).OrderByDescending(tp => tp.Volume24h).Take(limit).ToListAsync();
    }

    public async Task<long> GetTradingPairIdAsync(string symbol)
        => (await GetBySymbolAsync(symbol))?.Id ?? 0;
    ...
    public async Task<IEnumerable<TradingPair>> SearchTradingPairsAsync(string searchTerm, int limit)
    {
        return await _dbContext.Set<TradingPair>().AsNoTracking()
            ...
            .OrderBy(tp => tp.Symbol).Take(limit).ToListAsync();
    }
}
