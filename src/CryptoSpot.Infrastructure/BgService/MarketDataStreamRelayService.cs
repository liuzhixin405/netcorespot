    ...
    public class MarketDataStreamRelayService : BackgroundService
    {
        ...
        private async Task RelayOrderBookAsync(OrderBookDelta delta, CancellationToken ct)
        {
            try
            {
                ...
                if (pushAsSnapshot)
                {
                    var mapped = _mapping.MapOrderBookLevels(delta.Bids, delta.Asks);
                    var bidDtos = mapped.bids.ToList();
                    var askDtos = mapped.asks.ToList();
                    await push.PushExternalOrderBookSnapshotAsync(delta.Symbol, bidDtos, askDtos, nowMs);
                    snapshotCache?.Update(delta.Symbol, delta.Bids, delta.Asks, nowMs);
                }
                else
                {
                    var mappedDelta = _mapping.MapOrderBookLevels(delta.Bids, delta.Asks);
                    var bidDeltaDtos = mappedDelta.bids.ToList();
                    var askDeltaDtos = mappedDelta.asks.ToList();
                    await push.PushOrderBookDeltaAsync(delta.Symbol, bidDeltaDtos, askDeltaDtos);
                }
        ...
        private async Task RelayKLineAsync(KLineUpdate k, CancellationToken ct)
        {
            try
            {
                ...
                await push.PushKLineDataAsync(k.Symbol, interval, new KLineDataDto
                {
                    Id = klineEntity.Id,
                    Symbol = k.Symbol,
                    TimeFrame = interval,
                    ...
                    Close = klineEntity.Close,
                    Volume = klineEntity.Volume,
                    OpenDateTime = klineEntity.OpenDateTime,
