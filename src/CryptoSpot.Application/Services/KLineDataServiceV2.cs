...
namespace CryptoSpot.Application.Services
{
    ...
    /// DTO K线数据服务实现
    /// </summary>
    public class KLineDataServiceV2 : IKLineDataServiceV2
    {
        private readonly IKLineDataService _klineDataService; // 修复: 注入域服务而不是自身接口
        private readonly IDtoMappingService _mappingService;
        private readonly ILogger<KLineDataServiceV2> _logger;

        public KLineDataServiceV2(
            IKLineDataService klineDataService, // 修复: 使用域服务
            IDtoMappingService mappingService,
            ILogger<KLineDataServiceV2> logger)
        {
